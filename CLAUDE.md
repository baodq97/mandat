...
- **Research before drafting.** A governed doc that charters new product surface or picks among design alternatives starts from evidence, not memory: a research artifact (peer survey, repo measurement, spike note) lands in `docs/research/` first and the doc cites it as a Source. Observation → draft with no research artifact is prescribing before diagnosing (LEARNING-LOOP entry 17). Small mechanical stories with no design freedom are exempt — when in doubt, research.
- **Before proposing any status advance, red-team the doc**: an independent agent — never the doc's author, senior model tier — runs the spec-red-team pass (steelman → falsifiable "Fails if" findings → self-refutation → one kill criterion) and its brief goes to the owner together with the ratification request. The pass also verifies every cited Source — including the `docs/research/` artifact — actually exists. Advisory by construction: keyed passes never enter CI, hooks, or exit codes, and the owner may advance with findings open.
...
- **Skills are two-tier.** Tier 1: small, dependency-free skills that do one job independently and load on demand (spec-red-team, spec-author, better-wording, simplify-diff); agent definitions carry *hints* naming which Tier-1 skills to load, with the procedure embedded as fallback. Tier 2: orchestration — the lead composes Tier-1 workers through Workflow scripts (`.claude/workflows/`) or orchestrator skills. Tier-1 skills never depend on each other or on Tier 2; anything that coordinates is Tier 2 by definition.
- **Pre-PR self-review is mandatory.** Whoever authored a diff — the lead, an implementer subagent, or the product's dev RoleAgent via its playbook — runs the `simplify-diff` pass (reuse, dead code, comment policy incl. cited-doc-ids-exist, gofmt, scope) before the PR opens. Self-review is the author's floor; it never replaces the independent reviewer.
- **Release reconciliation is mandatory.** A promote/release is not done until a `doc-keeper` pass reconciles the governed docs against what shipped: which US/RFC/ADR do these changes complete, advance, or contradict? The pass proposes flips and corrections with evidence; flips still land only as human-authorized accept commits. An AC nothing implements is drift to surface, never silence (LEARNING-LOOP entry 16: ten chartering US sat open through ten releases because no ritual asked).
- **Milestone releases only** (owner ruling 2026-07-16). A tag ships when the bar clears: at least one US completed end to end, gates green, reconciliation done, release notes naming what changed per US. Routine merges accumulate on main. The pilot VM takes interim binaries through the dev channel — `CGO_ENABLED=0 GOARCH=arm64 go build`, scp, staged rename, restart the unit — never a tag. One exception ships immediately: a critical or security fix.
