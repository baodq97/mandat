...
// AgentUserName is the agent user's tracker handle — its UPN, the value ADO
// stores in System.AssignedTo — for whatever role this adapter instance acts
// as: Poll filters WIQL assignment on it for a dev-role adapter, while a
// reviewer-role probe instance (FindPR only) never reads it. It is NOT the
// Entra object id used to mint tokens. Tokens and Remits are required
// injected seams; HTTPClient and Logger default when nil.
type Config struct {
	BaseURL       string
	Org           string
	Project       string
...
func New(cfg Config) (*Adapter, error) {
	...
	if cfg.AgentUserName == "" {
		missing = append(missing, "AgentUserName")
	}
	if cfg.Tokens == nil {
