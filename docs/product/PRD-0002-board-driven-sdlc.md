...
| AC provenance on every slice card | every slice card names the doc AC-ids it derives from, so the doc stays the single authored spec | card-provenance audit: does each active-slice card cite its source AC-ids (research doc, Principle 1; probe `docs/research/ac-ownership-probe.md`) |
