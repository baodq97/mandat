...
Everything on the design's ordered roadmap is an explicit non-goal for the MVP, deferred not dropped: Arc identity mode, the SA-agent that would automate the manual decomposition above, the QA and Reviewer roles with scoring calibrate, the planning view with scheduled waves, Teams notification, the Jira adapter, agent-user assignment UX, and the multi-VM runner pool. The runner pool waits until a real customer saturates one VM, per the accepted single-VM trade-off (§12); the others follow in the §10 order once the MVP holds. One more deferred item sits behind the same seam D6 chose: support for coding tools beyond Claude Code (Codex, OpenCode, Gemini CLI) as alternative runners — the subprocess-plus-ResultContract interface is runner-agnostic by design (§12), so a new tool is a new runner config, never a new plane. The install experience already points that way in the MVP: `mandat init` treats the coding tool as a connection to configure (which tool, which auth mode — logged-in CLI, OAuth token, or API key), and `mandat doctor` proves the connection with a real headless call before first dispatch (§4.10).
...
| Anthropic auth for the runner (needed before S4 and pilot): the owner's existing Claude subscription, wired headless via a Claude Code OAuth token (`claude setup-token`) | available | token minted and a headless `claude -p` call succeeds on the VM as a non-login user; quota and rate-limit windows are shared with the owner's interactive use, so the budget circuit breaker (§4.3) is the guard, and a dedicated API billing account is deferred to the first non-dogfood pilot |
