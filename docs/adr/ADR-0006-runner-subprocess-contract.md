...
- **Version floor.** Require CLI ≥ 2.1.208, which fixes a truncated final `result` line on large piped output that would corrupt the file-contract read. Feature-detect via the init event's `capabilities[]` rather than comparing version strings. `mandat doctor` asserts the `claude` binary presence and version before first dispatch (§4.10).
